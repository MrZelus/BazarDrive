...
|---|---|---|---|---|
| 1) Theme tokens are centralized and used consistently. | **P0** | `web/js/tailwind-config.js` is the single source of truth for semantic theme tokens; `Feed/Rules/Profile` no longer introduce hardcoded color values or ad-hoc color classes outside approved token mapping. | Code scan of changed files for hardcoded hex/rgb/tailwind color literals; targeted UI smoke on `Feed/Rules/Profile` in desktop/mobile and Chrome/Edge to confirm no visual breakage after token normalization. | PR with token normalization + migration diff, plus token usage audit note (before/after findings). |
