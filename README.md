...
#### Доступ через точку доступа (Wi‑Fi hotspot) устройства-сервера

Если вы раздаёте интернет с того же устройства, где запущены API и статика (hotspot), `localhost` будет работать **только на самом сервере**. Для второго телефона/ноутбука в hotspot-сети используйте IP сервера в подсети точки доступа.

1. Запустите сервисы с привязкой к `0.0.0.0`:
...
2. Узнайте IP сервера в hotspot-сети (обычно `192.168.43.1`, `192.168.137.1`, `172.20.10.1` или похожий).
3. На клиентском устройстве, подключённом к этой точке доступа, открывайте страницу с **тем же IP** и для статики, и для `apiBase`:

```text
http://<HOTSPOT-IP-СЕРВЕРА>:8000/public/guest_feed.html?apiBase=http://<HOTSPOT-IP-СЕРВЕРА>:8001
```

Пример: `http://192.168.43.1:8000/public/guest_feed.html?apiBase=http://192.168.43.1:8001`.

Если оставить `apiBase=http://localhost:8001`, клиентское устройство будет обращаться к **самому себе**, а не к серверу hotspot.
