    ...
    const docs = [
      ...
      {
        id: 'platform-rules',
        title: 'Правила заказа такси и доставки пассажиром',
        description: 'Условия оформления, отмены и безопасного выполнения заказа в BazarDrive для пассажиров.',
        tags: ['пассажир', 'заказ', 'доставка', 'отмена', 'безопасность', 'пдд', 'статус'],
        type: 'Регламент',
        summary: 'Пассажир указывает точные данные заказа, соблюдает правила безопасности и несёт ответственность за передаваемые вещи.',
        sections: [
          {
            title: '1. Общие положения',
            items: [
              'Создавая заказ в BazarDrive, пассажир подтверждает согласие с правилами платформы, условиями выполнения заказа и требованиями безопасности.',
              'Заказ считается оформленным после передачи данных в систему и становится доступным водителям для принятия.',
            ],
          },
          {
            title: '2. Оформление заказа такси',
            items: [
              'Обязательно указываются адрес подачи, адрес назначения, контактный телефон и комментарий при необходимости.',
              'Для отложенного заказа дополнительно указывается время поездки.',
              'Особые условия нужно указать заранее: ребёнок/детское кресло, животное, крупный багаж, помощь пассажиру, промежуточные остановки.',
            ],
          },
          {
            title: '3. Доставка пассажиром (передача вещей)',
            items: [
              'При доставке необходимо указать состав отправления, адрес отправки и получения, контакты отправителя и получателя.',
              'Запрещено передавать предметы, запрещённые законом, включая оружие, наркотические средства, токсичные и взрывоопасные материалы.',
            ],
          },
          {
            title: '4. Обязанности пассажира',
            items: [
              'Указывать достоверные данные заказа.',
              'Быть готовым к поездке ко времени подачи и отвечать на звонки/сообщения.',
              'Соблюдать правила поведения и оплачивать заказ.',
            ],
          },
          {
            title: '5. Подача автомобиля и ожидание',
            items: [
              'После принятия заказа водитель направляется к месту подачи.',
              'Пассажир обязан выйти к автомобилю вовремя и не задерживать отправление.',
              'Если пассажир не выходит, не отвечает или задерживает поездку, водитель вправе отменить заказ, зафиксировать ожидание и применить условия сервиса.',
            ],
          },
          {
            title: '6. Изменение маршрута',
            items: [
              'Маршрут можно изменить до начала поездки или во время поездки по согласованию с водителем.',
              'Изменение маршрута может повлиять на стоимость, время и возможность выполнения заказа.',
            ],
          },
          {
            title: '7. Отмена заказа',
            items: [
              'До назначения водителя заказ можно отменить без ограничений.',
              'После назначения водителя отмена фиксируется системой.',
              'Частые отмены могут приводить к ограничениям, снижению приоритета и системным уведомлениям.',
            ],
          },
          {
            title: '8. Поведение в поездке',
            items: [
              'Запрещено мешать управлению автомобилем, требовать нарушать ПДД, оскорблять водителя или портить имущество.',
              'Запрещена перевозка запрещённых предметов.',
            ],
          },
          {
            title: '9. Безопасность',
            items: [
              'Пассажир обязан пристегнуться ремнём, соблюдать требования водителя и не отвлекать его во время движения.',
            ],
          },
          {
            title: '10. Основания для отказа',
            items: [
              'Водитель может отказаться от заказа при ложных данных, отсутствии пассажира, угрозе безопасности или нарушении правил.',
            ],
          },
          {
            title: '11. Ответственность',
            items: [
              'Пассажир несёт ответственность за корректность данных, поведение, причинённый ущерб и перевозимые предметы.',
            ],
          },
          {
            title: '12. Право платформы',
            items: [
              'BazarDrive вправе ограничивать доступ к заказам, учитывать историю пользователя, блокировать при нарушениях и изменять правила.',
            ],
          },
          {
            title: 'UX-версия (для фронта / Telegram)',
            items: [
              '🚕 BazarDrive — правила пассажира: указывайте точные адреса, выходите вовремя и заранее сообщайте о багаже, животных или особых условиях.',
              'Запрещены ложные заказы, агрессия, опасные предметы и требования нарушать ПДД. Частые отмены могут привести к ограничениям.',
            ],
          },
          {
            title: 'Мини-версия (карточка)',
            items: [
              'Будьте вовремя, указывайте точные данные и не нарушайте правила — это влияет на доступ к заказам.',
            ],
          },
        ],
      },
      {
        id: 'driver-documents-checklist',
        title: 'Чек-лист документов водителя',
        description: 'ВУ, СТС, ОСАГО (такси), ОСГОП, разрешение перевозчика, путевой лист, диагностическая карта.',
        tags: ['документы', 'осаго', 'осгоп', 'путевой лист'],
        type: 'Чек-лист'
      },
      {
        id: 'ip-usn',
        title: 'Сведения ИП на УСН',
        description: 'ИНН, ОГРНИП, дата регистрации, налоговый режим и регион работы.',
        tags: ['ип', 'усн', 'инн', 'огрнип'],
        type: 'Документ'
      },
      {
        id: 'order-log',
        title: 'Журнал регистрации заказов',
        description: 'Номер заказа, таймлайны, адреса, авто/водитель, канал заказа, контакт пассажира и доптребования.',
        tags: ['журнал', 'заказы', 'адреса', 'таймлайны'],
        type: 'Шаблон'
      },
      {
        id: 'security-payments',
        title: 'Безопасность и выплаты',
        description: 'Реквизиты для выплат, 2FA, контроль активных устройств и проверка статусов в профиле.',
        tags: ['безопасность', '2fa', 'выплаты', 'реквизиты'],
        type: 'Памятка'
      }
    ];
    ...
    function renderDriverHeaderAndQuickStatuses() {
      ...
      if (driverProfileHeaderMeta) {
        ...
        if (headerUiState === 'loading') {
          driverProfileHeaderMeta.textContent = 'Обновляем профиль, роль и статусы…';
      ...
      if (driverProfileHeaderStatusChip) {
        ...
        if (headerUiState === 'loading') {
          renderStatusChip(driverProfileHeaderStatusChip, 'checking', { label: 'Статус: обновляем данные' });
        } else if (headerUiState === 'error') {
          renderStatusChip(driverProfileHeaderStatusChip, 'rejected', { label: 'Статус: ошибка' });
        } else if (headerUiState === 'empty') {
          renderStatusChip(driverProfileHeaderStatusChip, 'profile_incomplete', { label: 'Статус: профиль не заполнен' });
        } else {
    ...
    function renderDriverDocuments(items = []) {
      ...
      const labels = {
        ...
        osgop: 'ОСГОП', diagnostic_card: 'Диагностическая карта', self_employed_certificate: 'Справка самозанятого'
      };
