    ...
    const docs = [
      ...
      {
        id: 'passenger-safe-ordering-rules',
        title: 'Что пассажиру важно по закону при заказе такси (РФ)',
        description: 'Что пассажиру важно по закону при заказе такси в РФ (актуально на 29.03.2026).',
        tags: ['пассажир', 'фз-580', 'пдд', 'зозпп', 'жалобы', 'безопасность'],
        type: 'Памятка',
        summary: 'Проверяйте легальность такси и данные заказа, соблюдайте ПДД и сохраняйте подтверждения оплаты для защиты прав.',
        sections: [
          {
            title: 'Что пассажиру важно по закону',
            orderedItems: [
              'Проверяйте легальность машины до посадки: у такси должны быть обязательные признаки и информация в салоне (госномер, перевозчик, разрешение, контакты для жалоб, Ф. И. О. водителя) — ФЗ №580, ст. 9.',
              'Сверяйте данные машины и водителя с приложением: не садитесь, если номер, марка или водитель не совпадают с заказом.',
              'Пристёгивайтесь обязательно: по ПДД пассажир обязан использовать ремень безопасности (п. 5.1), а также не отвлекать водителя и не открывать двери во время движения (п. 5.2).',
              'Если едете с ребёнком, указывайте это при заказе: применяются требования п. 22.9 ПДД, поэтому опцию «детское кресло» лучше выбирать заранее.',
              'Сохраняйте подтверждение поездки и оплаты: чек, квитанция и данные заказа важны для претензий, возврата и споров.',
              'При некачественной услуге используйте защиту прав потребителя: по Закону о защите прав потребителей можно требовать устранения нарушений, уменьшения цены или возврата денег (в зависимости от ситуации).',
              'Подавайте жалобы по цепочке: сначала в поддержку агрегатора или перевозчика, затем в уполномоченный орган по такси субъекта РФ, по потребительским нарушениям — в Роспотребнадзор, при угрозе безопасности — 112/полиция.',
            ],
          },
        ],
      },
